...
BUILD_HOSTS="x86_64-w64-mingw32 x86_64-linux-gnu"
...
for host in $BUILD_HOSTS; do
    ...
    if [ "$host" == "x86_64-w64-mingw32" ]; then # Windows x86_64. => xxx.zip
        ...
        cp -r "$BUILD_TEMP_DIR/release/share" "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR" || exit 1
        if [ -f "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/bin/depinc-cli" ]; then
            printf '#! /bin/sh\n./bin/depinc-cli -datadir=./data/ "$@"' > "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/depinc-cli.sh"
            chmod +x "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/depinc-cli.sh"
        fi
        if [ -f "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/bin/depincd" ]; then
            printf '#! /bin/sh\n./bin/depincd -datadir=./data/ "$@"' > "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/start-depincd.sh"
            printf '#! /bin/sh\n./bin/depinc-cli -datadir=./data/ "$@" stop' > "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/stop-depincd.sh"
            chmod +x "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/start-depincd.sh"
            chmod +x "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/stop-depincd.sh"
        fi
        if [ -f "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/bin/depinc-qt" ]; then
            printf '#! /bin/sh\n./bin/depinc-qt -datadir=./data/ "$@"' > "$BUILD_TEMP_DIR/release/$APPNAME-$APP_VER_SHORTSTR/start-depinc-qt.sh"
